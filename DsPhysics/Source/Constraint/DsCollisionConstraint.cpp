...
void DsCollisionConstraint::SetUp()
{
	...
	//è’ìÀì_ÇPÇ¬ÇPÇ¬Ç…ëŒÇµÇƒçSë©âÇ≠ÇÒÇæÇ¡ÇΩÇÁÇÕÉoÉlÉ_ÉìÉpÇ≈íµÇÀï‘Ç∑ï˚Ç™ÉRÉXÉgìIÇ…ó«Ç¢ÅB
	//ìZÇﬂÇƒâÇ≠ï˚ñ@ÇÕÇ»Ç¢Ç©ÅHÅ©ìZÇﬂÇƒÇ‡åãâ ÇÕìØÇ∂
	//ODEÇÃÉ\Å[ÉXÇ∆Ç©éQçlÇ…ÇµÇƒÇ›ÇÈ
	//ë¨ìxÇ∆óÕÇ™è¨Ç≥Ç¢Ç»ÇÁëäëŒë¨ìxÅÅÇOÇ≈çSë©èåèÇâÇ¢ÇƒÇ›ÇÈÅB
	...
	if (pMas && pSub){
		if (!m_isSetUp){
			...
			if (((0.0000001 > pMas->RefMaterial().m_bundCoef) && (pMas->RefMaterial().m_bundCoef < -0.0000001)) ||
				...
				m_bound = 0;//Ç«Ç¡ÇøÇ©Ç™ÇOÇ»ÇÁíµÇÀï‘ÇÁÇ»Ç¢
			}
			...
			if (((0.0000001 > pMas->RefMaterial().m_staticFrictionCoef) && (pMas->RefMaterial().m_staticFrictionCoef > -0.0000001)) ||
				...
				m_sFric = 0;//Ç«Ç¡ÇøÇ©Ç™ñÄéCñ≥éãÇ»ÇÁÇO
			}
			...
			if (((0.0000001 > pMas->RefMaterial().m_kinematicFricCoef) && (pMas->RefMaterial().m_kinematicFricCoef > -0.0000001)) ||
				...
				m_kFric = 0;//Ç«Ç¡ÇøÇ©Ç™ñÄéCñ≥éãÇ»ÇÁÇO
			}
			...
			//çSë©ç¿ïWånÇ©ÇÁï®ëÃç¿ïWånÇ…ïœä∑Ç∑ÇÈçsóÒÅBè’ìÀêÍópÇ»ÇÃÇ≈è’ìÀï˚å¸ÇµÇ©ê¨ï™Ç™Ç»Ç¢1*12
			...
			//double check[12][3];
			//DsMathUtil::MultiMatMN<12, 3, 12>(check, m_invM, m_Jt);//m_invMÇ™ëŒäpê¨ï™ÇµÇ©Ç»Ç¢ÇÃÇ≈è»ÇØÇÈÇÃÇ≈ÇÕÅH
...
void DsCollisionConstraint::_SetMass(const DsActor& master, const DsActor& sub)
{
	...
	//ÇQå¬ÇÃï®ëÃÇÃèdÇ≥Ç12*12ÇÃëŒäpçsóÒÇ…äiî[
	...
	//DsInverseMatrix<12>(m_M, m_invM);//ëŒäpê¨ï™ÇµÇ©Ç»Ç¢ÇÃÇ≈ÅAå¬ï Ç…ãÅÇﬂÇΩãtçsóÒÇåqÇ∞ÇÈÇ≈OK
...
{
	//Ç±Ç±Ç≈ÅAçSë©ç¿ïWånÇ≈ÇÃçSë©èåè(äeé≤ÇÃë¨ìxÇ™0ÇæÇ∆Ç©)Çï®ëÃç¿ïWånÇÃçSë©óÕÇ…ïœä∑Ç∑ÇÈ
	double invMFe[12];
	//DsMathUtil::MultiVec<12, 12>(invMFe, m_invM, m_Fe);//m_invMÇ™ëŒäpê¨ï™ÇµÇ©Ç»Ç¢ÇÃÇ≈è»ÇØÇÈÇÃÇ≈ÇÕÅH
	...
	//JÇ™è’ìÀï˚å¸Ç…ÇµÇ©ëŒâûÇµÇƒÇ»Ç¢ÇÃÇ≈ÅAw[t+1]ÇÕè’ìÀï˚å¸ÇÃë¨ìxÅiÉXÉJÉâÅ[ÅjÇ…Ç»ÇÈ
	...
	//Ç±ÇÍÇ™ç≈èIìIÇ…ãÅÇﬂÇΩÇ¢éÆ
	//x = (w[t+1]-b)/A
	//x=lambda Ç∆ÇµÇƒÅ´ãLèq

	//w[t+1] ÇÕéüÇÃñ⁄ïWë¨ìxÇ»ÇÃÇ≈ÅA
	//w[t+1] = w(ç°ÇÃë¨ìx)*e(íµÇÀï‘ÇËåWêî) Ç∆ÇµÇƒâÇ≠
	...
	if (pMas && pSub){
		if (pMas->RefOption().isStatic || pSub->RefOption().isStatic
			...
			staticC = 2.0;//àÍï˚Ç™ê√ìIÇ»èÍçáÅAìÆÇ©Ç»Ç¢ÇÃÇ≈ÅAìÆÇ≠ÇŸÇ§ÇÇQî{
	...
	requestVel = ((20.0 * m_error) - (0.05*m_w[2]));//ÇﬂÇËçûÇ›âè¡ë¨ìx
	if (requestVel < 0.0){
		requestVel = 0.0;
	}
	...
	{//ñÄéC
		const bool isDown = (JinvMFe[2] < 0.0);//íæÇﬁï˚å¸Ç…óÕÇ™â¡ÇÌÇ¡ÇƒÇÈ
		if (isDown){
			...
			if (isStop || (Fxy < nFs*nFs) ){//ê√é~ñÄéC
				m_cEq.isStaticByFric[0] = true;//êÇíºçRóÕ*ñÄéCåWêîÇÊÇËí·Ç¢óÕÇµÇ©Ç©Ç©Ç¡ÇƒÇ»Ç¢ÇÃÇ≈ê√é~Ç≥ÇπÇÈ
				m_cEq.isStaticByFric[1] = true;
				//ë¨ìx0
				m_cEq.min_wplus1[0] = 0; m_cEq.min_wplus1[1] = 0;
				m_cEq.max_wplus1[0] = 0; m_cEq.max_wplus1[1] = 0;
			}
			else if ( DBL_EPSILON < Vxy){//ìÆñÄéC
				...
				DsVec2d fricF = -DsVec2d(b[0] / VxySp, b[1] / VxySp);//ë¨ìxÇ∆îΩëŒï˚å¸Ç…Ç©ÇØÇÈ

				//åªç›ÇÃë¨ìxÇí¥Ç¶Ç»Ç¢ÇÊÇ§Ç…
			...
			//ïÇÇ≠ï˚å¸Ç…óÕÇ™â¡ÇÌÇ¡ÇƒÇ¢ÇÈÇÃÇ≈ñÄéCñ≥Çµ
			m_cEq.isStaticByFric[0] = false;
			m_cEq.isStaticByFric[1] = false;
	...
	//memcpy(m_cEq.A, m_JinvMJt, sizeof(double) * 9);//ÉZÉbÉgÉAÉbÉvà»ç~ïœÇÌÇÁÇ»Ç¢ÇÃÇ≈ÅB
...
void DsCollisionConstraintSolver::Solve(DsCollisionConstraint& c, const int iterationNum)
{
	...
	const bool isUseInverseMat = false;//è’ìÀÇÕÇ™ÇÒÇŒÇ¡ÇƒíµÇÀï‘ÇËåWêîï™ÇÃë¨ìxèoÇªÇ§Ç∆Ç∑ÇÈÇ∆ÅAä±è¬ÇÕçló∂Ç≥ÇÍÇƒÇ»Ç¢ÇÃÇ≈î≠éUÇµÇ‚Ç∑Ç¢ÅBåJÇËï‘ÇµÇ≈í˙ÇﬂÇÈÇÊÇ§Ç…ÇµÇΩï˚Ç™Ç¢Ç¢Ç©Ç‡ÅB
	...
	if (isUseInverseMat){
		...
		for (int iteIdx = 0; iteIdx < iterationNum; ++iteIdx){
			...
			//ÇPçsÇ†ÇΩÇË3*3Ç†ÇÈÇÃÇ≈ÇªÇÍÇâÇ¢ÇƒÇ¢Ç≠ÅBAÇÃíÜÇÃçsÉãÅ[Év
			for (int row = 0; row < 3; ++row){
				//åvéZÇ∑ÇÈçSë©ÇÃé≤ÇîªíËÇ∑ÇÈÅB2(z)ÇÕè’ìÀï˚å¸Ç»ÇÃÇ≈ê‚ëŒÅB1,2ÇÕê√é~ñÄéCÇè„âÒÇ¡ÇΩäOóÕÇ™â¡ÇÌÇ¡ÇΩÇ©Ç≈åvéZÇÃóLñ≥åàÇﬂÇÈ
				if ((row == 2) || (eq.isStaticByFric[row])){
					//è’ìÀóÕ
					if (0.0000001 < fabs(eq.A[row][row])){
						double forward = 0;//çXêVçœÇ›K+1
						for (int col = 0; col < row; ++col){//àÍÇ¬àÍÇ¬ÇÃAê¨ï™ÇÃóÒÇÃÉãÅ[ÉvÅBforwardÇÕÅAä˘Ç…âÇ¢ÇƒÇ†ÇÈrowê¨ï™Ç‹Ç≈â¡éZ
							forward += eq.A[row][col] * eq.lambda[col];
						}

						double back = 0;//ñ¢çXêVï™k
						for (int col = row + 1; col < 3; ++col){ //forward+é©ï™ÇÉXÉLÉbÉvÇµÇƒÇªÇÍà»ç~ÇÃóÒÇâ¡éZ
						...
						eq.lambda[row] = old_lamdba + weight*(new_lambda - old_lamdba);//SORñ@ÅBÉKÉEÉXÉUÉCÉfÉãÇÃâÇÃïœâªó Çweightî{â¡ë¨
